...
def newick2nodes(tree,rprint):
    ...
    for i in range(n_nodes):
        ...
        if i<len(leaves):  # If leaves
            ...
            nodes[i].label=str(i+1)
    clades=re.findall('[a-zA-Z\/:0-9.,-_]+',tree) # 
    acc=len(leaves)
    tmp=0
    t=tree
        ...
        for p in c:
            if p!='':
                ...
                if m==None:
                    ...
                    t=re.sub(str(p[0]+':'+p[1]),str(acc),t,1)
                else:
                    tmp=tmp+1
...
def taxa2nodes(tree,rprint):
    ...
    ### It is the same up to here
    ### It changes from here              
                   next_p=str(unichr(int(ord(p_cl[0]))+1))
