...
# This script is originally obtained from https://github.com/lehner-lab/DMS2structure/tree/master/scripts/XPLOR
# 
...
# also be specified.
#
args=xplor.parseArguments()
seqfile=args[1][0]
tblfile=args[1][1]
dihefile=args[1][2]
mypottype = args[1][3]  # Soft or hard
folder=args[1][4]
nmodel=int(args[1][5])
topavg=float(args[1][6])
...
# but it is optional.
#
outFilename = folder+"/SCRIPT_STRUCTURE.sa"
numberOfStructures=nmodel   #usually you want to create at least 20 
...
for (name,scale,file) in [('all',1,tblfile),
                          #add entries for additional tables
                          ]:
    pot = create_NOEPot(name,file)
    pot.setPotType(mypottype) # if you think there may be bad NOEs
    pot.setScale(scale)
    noe.append(pot)
...
dihedralRestraintFilename=dihefile
...
StructureLoop(numStructures=numberOfStructures,
              ...
              averageTopFraction=topavg, #report stats on best 5% of structs
